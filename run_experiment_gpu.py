...
def plot_summary_visuals(
    class_save_dir, class_name, image_roc_auc, fpr, tpr, gt_labels, img_scores
):
...
def plot_individual_visualizations(
    test_images, raw_maps, norm_scores, img_scores, save_dir, test_filepaths
):
    ...
    for i in range(len(test_images)):
        ...
        axes[3].text(
            5, 20, f"Score: {score:.3f}", color="white", backgroundcolor="black"
        )
...
def run_class_processing(args, class_name, model, device, random_feature_indices):
    ...
    mean_gpu = torch.tensor(learned_distribution[0], device=device, dtype=torch.float32)
    cov_gpu = torch.tensor(learned_distribution[1], device=device, dtype=torch.float32)
    ...
    for image_batch, labels, _ in test_dataloader:
        ...
        diff = embedding_vectors_batch - mean_gpu  # Broadcasting handles the math here
    ...
    # Concatenate the small distance results from all batches
    ...
    plot_mean_anomaly_maps(
        class_save_dir, ground_truth_labels, anomaly_maps_raw, score_maps
    )
...
def main():
    ...
    for class_name in bowtie.CLASS_NAMES:
        try:
            ...
            logging.error(
                f"!!! FAILED to process class {class_name}. Error: {e}", exc_info=True
            )
